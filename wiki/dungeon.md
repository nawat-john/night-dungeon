...
Each room generated by BSP is assigned an **archetype** that determines its contents and rules. The director weights room types per floor number (early floors are gentler; deeper floors have more arenas and hazard rooms).

| Archetype | Contents | Rules |
|---|---|---|
| **Vault** | Locked chest in the centre | Requires a Lockpick or champion key to open |
| **Arena** | Empty on entry — doors seal when player enters | 3 enemies spawn; doors unseal + chest drops on full clear |
| **Shrine** | Glowing shrine interactable | One-time buff (HP/MP restore) or risky bargain (trade HP for buff) |
| **Library** | Lectern interactable | Research clue, recipe unlock, or investigation hint |
| **Rest Nook** | Safe rest spot | No enemies spawn; fatigue ticks down when inside |
| **Hazard** | Floor-appropriate hazards spawned throughout | No enemies — pure environmental threat (see Per-floor hazards) |
| **Puzzle** | Two pressure plates (orange + blue) | Step on both to open a reward chest; either plate alone does nothing |
| **Merchant Pocket** | In-floor vendor (rare stock) | Wanders Merchant anomaly equivalent; buy consumables mid-run |
| **Empty** | Nothing | Pacing / navigation room; occasional ambient enemies only |

---

## Per-Floor Environmental Hazards (P12)

In addition to hazards placed in Hazard archetype rooms, each floor has themed hazards distributed across the map.

| Floor | Theme | Hazards |
|---|---|---|
| 1 | Ruined Entrance | None |
| 2 | Flooded Halls | Water tiles (wet build-up 35/s; quenches burn; lightning ×2) |
| 3 | Fungal Depths | Gas/spore zones (poison 20/s) + explosive **puffballs** (step within range → 20 dmg + spore burst) |
| 5 | Ashen Foundry | **Fire** tiles (burn 30/s; 1 dmg/500ms) + **oil pools** (fire attacks ignite → fire hazard spreads) |
| 6 | Frozen Reliquary | **Ice patches** (slide inertia; chill 15/s) + **thin ice** tiles (collapse after 2 s standing → 20 fall dmg) + blizzard overlay (reduced visibility) |
| 7 | Shadowed Catacombs | **Curse fog** (corruption build-up 15/s) + **gravetide spawners** (spawn undead wave every 10–15 s) + **falling bricks** (ceiling collapses on F7+) |
| 8 | Voidtouched Caverns | **Gravity wells** (60 px pull radius; deals void dmg) + **drifting rifts** (moving void zones; 10 void dmg on contact) |
| 9–10 | Throne Halls | Mix of fire, gravity wells, and falling bricks |

> **Hazard room** for any floor spawns 2–4 of the floor-appropriate hazard type in a concentrated room for a focused environmental challenge.
...
Some walls are **cracked** — 2–3 per floor, hidden among normal cave walls. They can be revealed by:

- Equipping and using a **Detector Charm** (briefly pings nearby cracked walls within 6 tiles)
- Throwing an explosive barrel or bomb at a wall you suspect

Behind cracked walls: small alcoves with buried caches (loot, gold, or rare materials).

### Investigation Clues

**Lore pages** in Library rooms and **Research rank-ups** may add a **discovered clue** to your Account Meta. Clues can be reviewed in the **Adventurer's Guild → Investigation** tab across runs. Gathering enough clues reveals the conditions to summon secret bosses (Gravelord, Avarice, Clockwork Judge, Old Friend, Hungering Dark, Aurelion).
